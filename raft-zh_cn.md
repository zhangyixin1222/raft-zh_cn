...
Raft 是一种为了对复制日志进行管理的一致性算法。它提供了和 Paxos 算法相同的功能和性能，但由于采用了与Paxos 不同的算法结构，使得 Raft 算法更加容易理解而且更易于构建实际的系统。为了提升可理解性，Raft 将一致性算法分解成了几个关键模块，例如领导者选举、日志复制和安全性。此外Raft还通过实施一个更强的一致性来减少需要考虑的状态的数量。根据对使用者研究的一个案例结果可以看出，Raft 算法比 Paxos 算法更容易让学生学习。通过利用重叠了大多数成员来保证了安全性，Raft 算法还包括了一个新的机制来允许集群成员的动态变更。

## 1 介绍

一致性算法允许一组机器作为一个整体进行工作，即使其中一些机器出现故障也能够继续工作下去。因此，一致性算法在构建可信赖的大规模软件系统中扮演着重要的角色。在过去的 10 年里，Paxos  算法统治着一致性算法这一领域：绝大多数的实现都是基于 Paxos 或者受其影响。同时 Paxos 也成为了教学领域里讲解一致性问题时的示例。

然而不幸的是，尽管有很多工作都在尝试降低它的复杂性， Paxos 算法依然十分难以理解。并且，Paxos 自身的算法结构需要进行大幅的修改才能够应用到实际的系统中。这些都导致了工业界和学术界都对 Paxos 算法感到十分头疼。

对 Paxos 算法做了一系列努力之后，我们开始寻找一种新的一致性算法，可以为构建实际的系统和教学提供更好的基础。我们的做法是不寻常的，我们的首要目标是可理解性：是否可以在实际系统中定义一个一致性算法，该算法能够比 Paxos 算法以一种更加容易的方式来学习。此外，我们希望该算法有助于系统构建者的基础性开发工作变得简单明了。不仅一个算法能够工作很重要，而且能够显而易见的知道为什么能工作也很重要。
...
*****以下译者注******
1、raft算法通过候选人资格的获取来剔除掉那些未持有有效最新日志的节点参与选举(即给其他节点发送广播自身的最新日志编号来获取是否有资格作为候选人，在收到大多数确认后才作为候选人发起选举)。
2、文中未对同一次崩溃后选举出双主问题进行讨论(即在及其特殊情况下，follower在通过c1提出的较小的term后又相继通过了c2提出的较大的term)，从而引起以下几种情况之一：a、c1与c2都认为自己是leader，但其实c2才是leader。b、c1获得绝大多数赞同票当选leader后发起新一轮提议并将提议发送到大多数节点，c2此时尚未获取大多数的赞同。对以上两种及其特殊情况要进行详加考虑。可以引入以下准则:选举中必须携带自身的最新提交日志的term作为工作term，follower仅接受大于等于自身认可term的提议，若提议小于自身日志term则不做处理，其他提议予以记录；收到提交信息后follower必须找到对应编号后无条件提交该提议(不管该提议在提议接受流程中是否曾被接受)；当候选人携带的工作term小于自身记录日志的term时，不投赞同票。
